...
export default function UploadSection() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(data as UploadResponse);
    } catch (err) {
  ...
  return (
    <section id="upload" className="scroll-mt-20 border-t border-border">
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-20">
        <h2 className="text-3xl font-semibold tracking-tight">Upload Your Dataset</h2>
        ...
        >
          <label htmlFor="dataset-file" className="block text-sm font-medium">
            Dataset file
          </label>
          <input
            id="dataset-file"
        ...
        {success ? (
          <div
            ...
            role="status"
          >
            <p className="font-medium text-brand-dark">{success.message}</p>
            <p className="mt-2 text-foreground">
              Filename: <span className="font-medium">{success.filename}</span>
            </p>
            <p className="mt-1 text-foreground">
              Dataset ID: <span className="font-mono text-xs sm:text-sm">{success.dataset_id}</span>
            </p>
          </div>
        ) : null}
      </div>
